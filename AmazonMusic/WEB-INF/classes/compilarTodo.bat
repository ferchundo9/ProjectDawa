javac businessLogic/Item.java
javac businessLogic/Cd.java
javac businessLogic/Tarjeta.java
javac businessLogic/Usuario.java
javac businessLogic/Cliente.java
javac businessLogic/Valoracion.java
javac businessLogic/Carrito.java
javac businessLogic/GestorCarrito.java
javac businessLogic/GestorProductos.java
javac businessLogic/GestorUsuarios.java
javac businessLogic/FachadaModelo.java


javac database/DAOInventario.java
javac database/DAOPedidos.java
javac database/DAOUsuarios.java
javac database/FachadaDAO.java

javac controllers/HelperCarrito.java
javac controllers/HelperProductos.java
javac controllers/HelperUsuarios.java
javac controllers/Controlador.java